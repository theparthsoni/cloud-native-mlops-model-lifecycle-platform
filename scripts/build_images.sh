...
docker build -t "$IMG_MLFLOW" -f "$ROOT_DIR/services/mlflow-image/Dockerfile" "$ROOT_DIR/services/mlflow-image"
...
docker build -t "$IMG_AIRFLOW" -f "$ROOT_DIR/services/airflow-image/Dockerfile" "$ROOT_DIR"
